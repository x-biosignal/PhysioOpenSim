...
2.  **Inverse Dynamics (ID)** – Compute joint moments from kinematics +
    forces
3.  **Static Optimization (SO)** – Estimate muscle forces from joint
    moments

PhysioOpenSim’s template system and tool runners make it straightforward
to script this pipeline entirely in R.
...
#> Applied tags: model_file, marker_file, output_motion_file, time_range, results_directory
```
...
#> Applied tags: model_file, coordinates_file, output_gen_force_file, lowpass_cutoff_frequency_for_coordinates, time_range
```
...
#> Applied tags: model_file, coordinates_file, time_range, results_directory
```
...
Native execution is faster but does not support `extra_args` or the
